module Genetic.ReadInstructions (readInstructions) where

import Data.Maybe
import Data.Char (toUpper, isDigit)
import Data.Array.IO (newListArray, IOArray)
...
stringToInstruction string =
       ...
       instruction =
          case words instr of
             ...
             ["SENSE", sensedir, st1, st2, "MARKER", i] ->
                Sense (readSenseDir sensedir) (readST st1) (readST st2) (Marker (readM i))
             ["MARK", i, st] ->
                Mark (readM i) (readST st)
             ["UNMARK", i, st] ->
                Unmark (readM i) (readST st)
             ["PICKUP", st1, st2] ->
  ...
  where
    ...
    readM = fromJust . flip lookup [(0, Zero),(1, One),(2, Two),(3, Three),(4,Four),(5,Five)] . (read :: String -> Int)
