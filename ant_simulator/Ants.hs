...
type Timing = Maybe (Int {-tick-}, Int {-roundnumber-}, UTCTime)
...
resetTiming ref =
   do rnr   <- readFromIORef ref (roundNumber . gameState)
      ctNow <- getCurrentTime --getClockTime
            ...
            do modifyIORef ref (\s -> s {timing = Just (tick+1, oldNr, ct)}) 
               when (tick `mod` timingFrequency == 0) $
                  do maxRound  <- readFromIORef ref (getNrOfRounds . options)
                     currRound <- readFromIORef ref (roundNumber . gameState)
                     ctNow     <- getCurrentTime -- getClockTime
                     let micros = convertTimeDiff (diffUTCTime ctNow ct)
...
convertTimeDiff :: NominalDiffTime -> Integer
convertTimeDiff tm = fst $ properFraction $ 1000000 * tm
